...
Trigger: reading an issue, creating an issue, posting/editing a comment, or transitioning an issue.
...
Do not: use a general-purpose assistant or other MCP server that exposes Jira write helpers. Those tools reformat submitted markup and append an attribution footer per call.
Exception: read-only fetches through another tool are acceptable when the Atlassian plugin is unavailable and the content is only being summarized, never written back.

### Rule: Match the markup to the write path's content format

Trigger: writing any text posted to a Jira Cloud comment or issue field.

Do: check whether the write tool takes a content-format parameter and match markup to it. Three paths, not interchangeable:

- Tool declares `contentFormat: markdown` — write Markdown. Suitable for ordinary comments: headings, bullets, inline code, fenced code blocks, links, bold.
- Tool declares `contentFormat: adf` — write Atlassian Document Format JSON. Use when comment needs fidelity Markdown cannot express (panels, statuses, expand blocks, language-set code blocks).
- Tool takes a raw body string with no content-format parameter — write Jira wiki markup using the token map below. Legacy REST v2 path.

Do: state the chosen format when handing a draft to a human, so the next agent doesn't send it through the wrong path.

Do not: mix two of the three in one body. Markdown on the wiki-markup path renders as a wall of raw symbols; wiki-markup on the Markdown path renders as literal `h2.` and `{{…}}` text.

Exception: if a human will paste into the Jira web editor manually, Markdown is fine regardless of path — the web editor converts paste-time Markdown locally. Label the draft "for manual paste".
...
Do not: reference files, paths, or artifacts only the author can access (local repo paths, private branches, personal machine paths). If a spec or design doc lives only locally, publish it first (Confluence, shared drive) and link the published version, or omit the reference.

Do: link Jira keys, PR URLs, published Confluence pages, and shared tickets by full URL or key. Prefer `[<label>|<url>]` over bare URLs when the label adds meaning.
...
Do: follow the template section-for-section. If it lives in project context (`.agents/context/` or a linked runbook), read it and reproduce the required sections in wiki markup. Include:
...
- Components affected.
- Manual verification results, own section with expected vs. actual per usecase.

Do not: invent template sections not in the documented template. If no template exists, ask before improvising.
...
Do: update the existing comment in place when the content is the canonical version. Preserves a single source of truth per ticket and avoids scroll-fatigue for reviewers.

Do not: post a new comment for every small revision. New comments are appropriate for distinct events (PR merged, deploy started, QA blocker found), not typo fixes or formatting corrections.
...
Do not: repeat "Requested by …" or "Posting on behalf of …" in the comment body. The tool footer identifies the requester; duplicating it clutters the comment.
