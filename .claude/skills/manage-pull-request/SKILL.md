...
8. Only after push verification, draft each PR reply in chat. Keep replies brief: acknowledge the point and state the fix in one or two sentences. When drafting replies, randomize their opening acknowledgements; do not begin every reply with the same phrase. For an improvement use `Thank you, concern addressed.`, `Thanks, concern addressed.`, `Appreciate it, concern addressed.`, or `You're right, concern addressed.` followed by the change. Acknowledgements may be combined naturally, for example `Thanks, you're right, concern addressed.` For a verified defect use the corresponding `issue confirmed and fixed` wording followed by the fix; `You're right, I have confirmed this issue and fixed it.` may stand alone. Do not mention a commit by default; cite a SHA or commit link only when the operator explicitly requests it. Do not restate the full analysis or call an issue confirmed unless validation established it as a defect.
