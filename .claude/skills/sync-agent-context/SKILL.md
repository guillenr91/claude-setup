...
`<target>/.gitignore` listing the paths sync-agent-context creates or modifies:

```
# BEGIN sync-agent-context (do not edit; regenerated by /sync-agent-context)
AGENTS.md
CLAUDE.md
.agents/**
.claude/**
.cursor/**
# END sync-agent-context
```
...
Do not edit content between the `# BEGIN sync-agent-context` and `# END sync-agent-context` markers
